...
function Update () {
	if(Input.GetButton("Crouch"))
	{
		...
		maincam.transform.position.y-0.3,
	...
	{
		if(controller.height != 2.3)
		{
			Physics.SphereCast(Vector3(transform.position.x,transform.position.y+0.3,transform.position.z),
			0.3,transform.up,hit,1);
			if(!hit.collider)
			{
				maincam.transform.position = Vector3(
				maincam.transform.position.x,
